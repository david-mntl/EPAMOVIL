...
public class EditCustomer extends ActionBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        customerID.setText(getIntent().getExtras().getString("userID"));
        ...
        if (c.moveToFirst()) {
            if(c.getString(11).compareTo("true") == 0) {
                ...
                new SweetAlertDialog(EditCustomer.this, SweetAlertDialog.ERROR_TYPE)
                        .setTitleText("Oops")
    ...
    private void deleteUserFromDatabase(){
        new SweetAlertDialog(EditCustomer.this, SweetAlertDialog.WARNING_TYPE)
        ...
        for(int i = 0; i <= info.length; i++){
            if( i == info.length){
                try {
                    SQLite user = new SQLite(EditCustomer.this, "DBClientes", null, 1);
                    SQLiteDatabase db = user.getWritableDatabase();
                    db.execSQL("DELETE FROM Customer WHERE CUSTOMER_ID=" + "\'" + pID +"\'");
                    ...
                    new SweetAlertDialog(EditCustomer.this, SweetAlertDialog.SUCCESS_TYPE)
